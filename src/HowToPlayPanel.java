...
public class HowToPlayPanel extends JPanel {
    ...
    private void createTopBar() {
        ...
        right.setPreferredSize(btnBack.getPreferredSize());
    ...
    private void createContentArea() {
        ...
        sideMenu.setBorder(BorderFactory.createEmptyBorder(10, 0, 0, 0));

        String[] topics = { "Objective", "Setup", "TurnFlow", "Tiles", "Cards", "Effects", "GameOver" };
        String[] titles = { "เป้าหมาย (Objective)", "เตรียมตัว (Setup)", "ลำดับการเล่น (Turn)", "ช่องเดิน (Tiles)",
                "การ์ด (Cards)", "เอฟเฟกต์ (Effects)", "แพ้ชนะ (Game Over)" };
        ...
        howToContentArea = new JEditorPane();
        howToContentArea.setContentType("text/html");
        howToContentArea.setEditable(false);
        howToContentArea.setBackground(new Color(255, 255, 255, 20));
        howToContentArea.setOpaque(false);
        howToContentArea.putClientProperty(JEditorPane.HONOR_DISPLAY_PROPERTIES, Boolean.TRUE);

        StringBuilder content = new StringBuilder();
        // Force ใช้ฟอนต์ Tahoma ใน HTML ถ้าไม่มี Mali
        String fontName = (GameTheme.THAI_FONT != null) ? GameTheme.THAI_FONT : "Tahoma";
        String css = "<style>body { font-family: " + fontName
                + ", sans-serif; color: white; margin: 20px; font-size: 14px; }" +
        ...
        for (String topic : topics) {
            content.append("<a name='").append(topic).append("'></a>").append(getSectionContent(topic))
                    .append("<br><hr><br>");
        }
        content.append("<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></body></html>");
        howToContentArea.setText(content.toString());
        howToContentArea.setCaretPosition(0);
        ...
        contentScroll.getViewport().setBackground(new Color(0, 0, 0, 100));
        contentScroll.setOpaque(false);

        contentContainer.add(menuScroll, BorderLayout.WEST);
    ...
    private JButton createSideMenuButton(String label, String topicKey) {
        ...
        btn.setFont(new Font(GameTheme.THAI_FONT, Font.BOLD, 16));
        btn.setHorizontalAlignment(SwingConstants.LEFT);
        btn.setFocusPainted(false);
        btn.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(GameTheme.WOOD_BORDER, 2),
                BorderFactory.createEmptyBorder(0, 15, 0, 0)));
        btn.setBackground(GameTheme.WOOD_COLOR);
    ...
    private void scrollToSection(String anchorName) {
        for (Map.Entry<String, JButton> entry : sideMenuButtons.entrySet()) {
            if (entry.getKey().equals(anchorName))
                entry.getValue().setBackground(GameTheme.ACTIVE_MENU_COLOR);
            else
                entry.getValue().setBackground(GameTheme.WOOD_COLOR);
        }
    ...
    private String getSectionContent(String topic) {
        switch (topic) {
            case "Objective":
                return "<h2>1. เป้าหมายของเกม (Objective)</h2><p>ผู้เล่นต้องพยายามเอาชนะคู่แข่ง...</p>";
            case "Setup":
                return "<h2>2. การเตรียมตัวเริ่มเกม</h2><ul><li>เงินตั้งต้น</li><li>รอบสูงสุด</li></ul>";
            case "TurnFlow":
                return "<h2>3. ลำดับการเล่น</h2><ol><li>ทอยเต๋า</li><li>เดิน</li><li>Action</li></ol>";
            case "Tiles":
                return "<h2>4. ประเภทช่องเดิน</h2><ul><li>ที่ดิน</li><li>ช่องพิเศษ</li><li>เสี่ยงดวง</li></ul>";
            case "Cards":
                return "<h2>5. ระบบการ์ด</h2><ul><li>Angel</li><li>Shield</li><li>...</li></ul>";
            case "Effects":
                return "<h2>6. เอฟเฟกต์พิเศษ</h2><ul><li>Olympic</li><li>Blackout</li></ul>";
            case "GameOver":
                return "<h2>7. การแพ้ชนะ</h2><ul><li>Victory Condition</li><li>Bankruptcy</li></ul>";
            default:
                return "<h2>" + topic + "</h2><p>Content goes here...</p>";
        }
    }
}