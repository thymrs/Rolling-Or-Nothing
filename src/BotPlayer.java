...
public class BotPlayer extends Player {
    ...
    public BotPlayer(int id, String name, int initialMoney, DifficultyLevel difficulty, DinoType dinoType) {
    ...
    public boolean makeDecision(DecisionType type, PropertyTile currentTile, GameState state) {
        return switch (this.difficulty) {
            case EASY ->
                handleEasyDecision(type, currentTile, state);
            case NORMAL ->
                handleNormalDecision(type, currentTile, state);
            case HARD ->
                handleHardDecision(type, currentTile, state);
            default ->
                false;
        };
    }
    ...
    public boolean handleNormalDecision(DecisionType type, PropertyTile tile, GameState state) {
        ...
        return switch (type) {
            case BUY_LAND ->
                evaluateNormalBuyLand(tile, state);
            case USE_CARD ->
                evaluateNormalUseCard(state);
            case UPGRADE ->
                (this.getMoney() > salary * 2) && chance(60); // ต้องมีเงินมากกว่า 2 เท่าของเงินเดือนถึงจะอัปเกรด
            case PAY_TOLL ->
                true;
            case SURRENDER ->
                this.getMoney() < 0;
    ...
    private boolean evaluateEasyUseCard() {
        ...
        if (isBestCase) {
            return (this.getMoney() < 200 || this.isJailed); 
        }else {
            return chance(50);
        }
    }
    ...
    private boolean evaluateHardBuyLand(PropertyTile tile, GameState state) {
        ...
        if (!isTakeover) {
            if (willCompleteSet(tile)) {
                return true;
            }
            return (this.getMoney() - price) >= 500;
        } else {
            // เทคโอเวอร์
            if (willCompleteSet(tile) || willBlockOpponent(tile, state)) {
                return true;
            }
            if (isRiskAhead(state)) {
                return false;
            }
    ...
    private boolean evaluateHardUseCard(GameState state) {
        ...
        if (isBestCase) {
            if (isOpponentCloseToVictory(state)) {
                return true;
            }
    ...
    private int getHardWorldTourDestination(GameState state) {
        ...
        if (bestUnowned != null) {
            return bestUnowned.getIndex();
        }
        if (bestOwnedToUpgrade != null) {
            return bestOwnedToUpgrade.getIndex();
        }
    ...
    public int chooseWorldTourDestination(GameState state) {
        return switch (this.difficulty) {
            case EASY ->
                getEasyWorldTourDestination(state);
            case NORMAL ->
                getNormalWorldTourDestination(state);
            case HARD ->
                getHardWorldTourDestination(state);
            default ->
                getEasyWorldTourDestination(state);
        };
    }
    ...
    private boolean willCompleteSet(PropertyTile tile) {
        ...
        for (PropertyTile land : this.getOwnedLands()) {
            if (land.getColorGroup().equals(color)) {
                countInSet++;
            }
        }
        ...
        for (int i = 2; i <= 12; i++) {
            ...
            if (targetTile instanceof PropertyTile property) {
                ...
                if (owner != null && owner != this && !property.isMortgaged()) {
                    if (property.calculateRent() > (this.getMoney() * 0.2)) {
                        return true;
                    }
                }
            }
